 ...
 * @author rafael
 */
@ManagedBean(name="listaproductores")
@SessionScoped

public class ListaProductores extends BeanBase {
    private List items=null;

    public ListaProductores() throws Exception{
        getLista();
    }
    ...
    public void getLista() throws Exception {
        ...
        try{
            ...
            Query q=session.createQuery("from Productor a order by a.nombre");
            Iterator i=q.list().iterator();
            
            while (i.hasNext()){
                   Productor p=(Productor) i.next(); 
                   item=new SelectItem();
