 ...
 * @author rafael
 */
@ManagedBean(name="listadepositos")
@SessionScoped

public class ListaDepositos extends BeanBase {
    private List items=null;

    public ListaDepositos() throws Exception{
        getLista();
    }
    ...
    public void getLista() throws Exception {
        ...
        try{
            ...
            Query q=session.createQuery("from Deposito a order by a.nombre");
            Iterator i=q.list().iterator();
            
            while (i.hasNext()){
                   Deposito p=(Deposito) i.next(); 
                   item=new SelectItem();
