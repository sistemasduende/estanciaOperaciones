 ...
 * @author rafael
 */
@ManagedBean(name="listaproveedores")
@SessionScoped

public class ListaProveedores extends BeanBase {
    private List items=null;

    public ListaProveedores() throws Exception{
        getLista();
    }
...
//        resultados.add(item);
        ...
        try{
            ...
            Query q=session.createQuery("from Proveedor a order by a.nombre");
            Iterator i=q.list().iterator();
            
            while (i.hasNext()){
                   Proveedor p=(Proveedor) i.next(); 
                   item=new SelectItem();
