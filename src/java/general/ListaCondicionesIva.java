 ...
 * @author rafael
 */
@ManagedBean(name="listacondicionesiva")
@SessionScoped

public class ListaCondicionesIva extends BeanBase {
    private List items=null;

    public ListaCondicionesIva() throws Exception{
        getLista();
    }
    ...
    public void getLista() throws Exception {
        ...
        try{
            ...
            Query q=session.createQuery("from CondIva a order by a.nombre");
            Iterator i=q.list().iterator();
            
            while (i.hasNext()){
                   CondIva p=(CondIva) i.next(); 
                   item=new SelectItem();
