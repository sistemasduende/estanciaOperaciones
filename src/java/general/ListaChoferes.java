 ...
 * @author rafael
 */
@ManagedBean(name="listachoferes")
@SessionScoped

public class ListaChoferes extends BeanBase {
    private List items=null;

    public ListaChoferes() throws Exception{
        getLista();
    }
    ...
    public void getLista() throws Exception {
        ...
        try{
            ...
            Query q=session.createQuery("from Chofer a order by a.nombre");
            Iterator i=q.list().iterator();
            
            while (i.hasNext()){
                   Chofer p=(Chofer) i.next(); 
                   item=new SelectItem();
