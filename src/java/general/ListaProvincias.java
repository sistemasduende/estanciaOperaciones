 ...
 * @author rafael
 */
@ManagedBean(name="listaprovincias")
@SessionScoped

public class ListaProvincias extends BeanBase {
    private List items=null;

    public ListaProvincias() throws Exception{
        getLista();
    }
    ...
    public void getLista() throws Exception {
        ...
        try{
            ...
            Query q=session.createQuery("from Provincia a order by a.nombre");
            Iterator i=q.list().iterator();
            
            while (i.hasNext()){
                   Provincia p=(Provincia) i.next(); 
                   item=new SelectItem();
